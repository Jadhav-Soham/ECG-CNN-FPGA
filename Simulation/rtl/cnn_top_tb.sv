module cnn_top #(
    ...
    // Debug ports — testbench observes intermediate values
 
    output logic signed [DATA_WIDTH-1:0] debug_fc_out  [NUM_CLASSES],
    output logic signed [DATA_WIDTH-1:0] debug_gap_out [CH3],

    // ECG BRAM write port — driven by testbench to load each test sample
    input  logic ecg_wr_en,
    input  logic [$clog2(CH0*ECG_LENGTH > 1 ? CH0*ECG_LENGTH : 2)-1:0] ecg_wr_addr,
    input  logic signed [DATA_WIDTH-1:0]  ecg_wr_data
);

// Derived feature-map lengths
...
// Wire internal arrays to debug output ports
assign debug_fc_out  = fc_out;
assign debug_gap_out = gap_out;

// BRAM INSTANCES

// ECG input BRAM (writable — testbench loads each sample via write port)
logic ecg_rd_en;
...
// CONV1  —  reads ECG BRAM, writes BRAM_12
...
conv1d_top #(
    ...
    .IN_LENGTH (LEN1),
    .OUT_LENGTH (LEN2),
    .WEIGHT_FILE ("conv2_weight.hex"),
    .BIAS_FILE ("conv2_bias.hex"),
    .RELU_EN (1)
) CONV2 (
    .clk (clk), .rst(rst),
...
conv1d_top #(
    ...
    .IN_LENGTH (LEN2),
    .OUT_LENGTH (LEN3),
    .WEIGHT_FILE ("conv3_weight.hex"),
    .BIAS_FILE ("conv3_bias.hex"),
    .RELU_EN (1)
) CONV3 (
    .clk (clk), .rst(rst),
...
// FC  —  reads gap_out via synchronous mux, writes fc_out registers
...
conv1d_top #(
    ...
    .IN_LENGTH (1),
    .OUT_LENGTH (1),
    .WEIGHT_FILE ("fc_weight.hex"),
    .BIAS_FILE ("fc_bias.hex"),
    .RELU_EN(0)
) FC (
    .clk (clk), .rst(rst),
...
// Top-level FSM  (one-hot encoding)
